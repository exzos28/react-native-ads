...
type Listener = (payload?: PangleAdEventPayload) => void;
...
class MockNativeAd {
  ...
  private listeners = new Map<PangleAdEventType, Set<Listener>>();
  private nextId = 1;
  private idToEntry = new Map<
    number,
    { type: PangleAdEventType; listener: Listener }
  >();

  addAdEventListener = jest.fn(
    (eventType: PangleAdEventType, listener: Listener) => {
  ...
  emit(eventType: PangleAdEventType, payload?: PangleAdEventPayload): void {
...
describe('InterstitialAd load/show lifecycle', () => {
  ...
  it('delegates to the native ad once loaded', async () => {
    ...
    await ad.show();
    expect(mockCreatedInterstitials[0]!.show).toHaveBeenCalledTimes(1);
  });
...
describe('MobileAds()', () => {
  it('reports a ready adapter status after initialize resolves', async () => {
    ...
    expect(statuses).toEqual([
      expect.objectContaining({ name: 'Pangle', state: 1 }),
    ]);
  });
});
