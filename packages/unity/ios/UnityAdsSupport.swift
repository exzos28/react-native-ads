...
let unityAdsErrorDomain = "NitroUnityAds"

func unityAdsError(_ message: String, code: Int = 1) -> NSError {
  NSError(
    domain: unityAdsErrorDomain,
    code: code,
...
func currentViewController() async throws -> UIViewController {
  try await withCheckedThrowingContinuation { continuation in
    DispatchQueue.main.async {
      ...
      guard let root else {
        continuation.resume(
          throwing: unityAdsError("Cannot show Unity ad without a view controller")
        )
        return
      }
...
/// Unity Ads load/show delegates are plain callback objects with no owner —
